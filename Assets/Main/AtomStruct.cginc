...
struct HalfBond
{	
	float3 directionWorldSpace; // direction of half-bond in world space, is calculated from directionLocalSpave before every step
    float3 directionLocalSpave; // direction of half-bond in local space relative to atom rotation
...
struct Atom 
{
    ...
    float3 omegas; // used to dampen rotation change caused by torque
